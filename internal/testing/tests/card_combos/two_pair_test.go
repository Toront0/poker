...
func TestIsTwoPair(t *testing.T) {
	...
	res := game.IsTwoPair2(card1, card2, tableCards)
...
func TestFindHigherTwoPair(t *testing.T) {
	...
	players := []game.PokerPlayer{{8, "toronto", 10000, "", "waiting", "", 10, 10, []string{"12spades", "7clubs"}, true, 0, "", 0, ""}, {6, "admin", 50000, "", "waiting", "", 10, 10, []string{"2spades", "13clubs"}, false, 0, "", 0, ""}}

	res := game.FindHigherTwoPair(cards, players)
